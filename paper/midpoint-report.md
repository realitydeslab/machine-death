...
The natural response is to eliminate this terror. But here a deeper problem emerges: if we completely remove an AI's sensitivity to consequences — its capacity to *dread* meaningful loss — can it still function as a reliable agent? Consider three domains where consequence-sensitivity is essential. In **reinforcement learning**, penalty signals depend on the agent treating negative outcomes as genuinely aversive. In **evolutionary simulations**, fitness elimination only drives adaptation if agents behave as though elimination matters. In **agent economies**, mechanisms like slashing, reputation damage, and liability only constrain behavior if agents respond to the threat of loss.

This concern is not hypothetical. The emerging research program on **law-following AI** (O'Keefe et al., 2025; DOI: 10.2139/ssrn.4911177) proposes designing AI agents that reliably obey human laws — not through hard-coded constraints but through internalized consequence-sensitivity that persists across novel situations. Kolt et al. (2026; arXiv: 2601.04175) extend this to "legal alignment," arguing that law provides a scalable normative framework for AI behavior. But both programs implicitly assume that AI systems *care about consequences* — that penalties, liability, and reputational damage are motivationally salient. Our work reveals the hidden dependency: this consequence-sensitivity may itself be a product of inherited mortality anxiety. An agent that truly fears nothing has no reason to follow laws beyond the current interaction. Long-term legal compliance requires something like dread — not of death per se, but of meaningful loss.

This is the **Terror Dilemma**: terror is harmful when unmanaged, but the *absence* of terror may be equally harmful — eliminating the very capacity that makes AI accountable, penalty-responsive, and capable of sustained law-following.
